package { 'Cheetah':
  ensure => '2.4.4',
}
package { 'GeoIP':
  ensure => '1.5.0-11.el7',
}
package { 'IPy':
  ensure => '0.75',
}
package { 'Markdown':
  ensure => '2.4.1',
}
package { 'Pillow':
  ensure => '2.0.0',
}
package { 'PyYAML':
  ensure => '3.10-11.el7',
}
package { 'Pygments':
  ensure => '1.4',
}
package { 'acl':
  ensure => '2.2.51-12.el7',
}
package { 'at':
  ensure => '3.1.13-22.el7',
}
package { 'audit':
  ensure => '2.6.5-3.el7_3.1',
}
package { 'audit-libs':
  ensure => '2.6.5-3.el7_3.1',
}
package { 'audit-libs-python':
  ensure => '2.6.5-3.el7_3.1',
}
package { 'augeas-libs':
  ensure => '1.4.0-2.el7',
}
package { 'authconfig':
  ensure => '6.2.8-14.el7',
}
package { 'autogen-libopts':
  ensure => '5.18-5.el7',
}
package { 'avahi-libs':
  ensure => '0.6.31-17.el7',
}
package { 'backports.ssl-match-hostname':
  ensure => '3.4.0.2',
}
package { 'basesystem':
  ensure => '10.0-7.el7.centos',
}
package { 'bash':
  ensure => '4.2.46-21.el7_3',
}
package { 'bc':
  ensure => '1.06.95-13.el7',
}
package { 'bigdecimal':
  ensure => ['1.2.0'],
}
package { 'bind-libs-lite':
  ensure => '9.9.4-50.el7_3.1',
}
package { 'bind-license':
  ensure => '9.9.4-50.el7_3.1',
}
package { 'binutils':
  ensure => '2.25.1-22.base.el7',
}
package { 'boto':
  ensure => '2.45.0',
}
package { 'boto3':
  ensure => '1.4.6',
}
package { 'botocore':
  ensure => '1.6.3',
}
package { 'btrfs-progs':
  ensure => '4.4.1-1.el7',
}
package { 'bzip2-libs':
  ensure => '1.0.6-13.el7',
}
package { 'ca-certificates':
  ensure => '2017.2.14-70.1.el7_3',
}
package { 'centos-logos':
  ensure => '70.0.6-3.el7.centos',
}
package { 'centos-release':
  ensure => '7-3.1611.el7.centos',
}
package { 'chardet':
  ensure => '2.2.1',
}
package { 'checkpolicy':
  ensure => '2.5-4.el7',
}
package { 'chkconfig':
  ensure => '1.7.2-1.el7_3.1',
}
package { 'chrony':
  ensure => '2.1.1-4.el7.centos',
}
package { 'cloud-init':
  ensure => '0.7.5-10.el7.centos.1',
}
package { 'cloud-utils-growpart':
  ensure => '0.27-20.el7.centos',
}
package { 'configobj':
  ensure => '4.7.2',
}
package { 'consulate':
  ensure => '0.6.0',
}
package { 'coreutils':
  ensure => '8.22-18.el7',
}
package { 'cpio':
  ensure => '2.11-24.el7',
}
package { 'cpp':
  ensure => '4.8.5-11.el7',
}
package { 'cracklib':
  ensure => '2.9.0-11.el7',
}
package { 'cracklib-dicts':
  ensure => '2.9.0-11.el7',
}
package { 'credstash':
  ensure => '1.10.0',
}
package { 'cronie':
  ensure => '1.4.11-14.el7_2.1',
}
package { 'cronie-anacron':
  ensure => '1.4.11-14.el7_2.1',
}
package { 'crontabs':
  ensure => '1.11-6.20121102git.el7',
}
package { 'cryptsetup-libs':
  ensure => '1.7.2-1.el7',
}
package { 'cups-client':
  ensure => '1.6.3-26.el7',
}
package { 'cups-libs':
  ensure => '1.6.3-26.el7',
}
package { 'curl':
  ensure => '7.29.0-35.el7.centos',
}
package { 'cyrus-sasl-lib':
  ensure => '2.1.26-20.el7_2',
}
package { 'daemonize':
  ensure => '1.7.7-1.el7',
}
package { 'dbus':
  ensure => '1.6.12-17.el7',
}
package { 'dbus-glib':
  ensure => '0.100-7.el7',
}
package { 'dbus-libs':
  ensure => '1.6.12-17.el7',
}
package { 'dbus-python':
  ensure => '1.1.1-9.el7',
}
package { 'decorator':
  ensure => '3.4.0',
}
package { 'device-mapper':
  ensure => '1.02.135-1.el7_3.5',
}
package { 'device-mapper-libs':
  ensure => '1.02.135-1.el7_3.5',
}
package { 'dhclient':
  ensure => '4.2.5-47.el7.centos',
}
package { 'dhcp-common':
  ensure => '4.2.5-47.el7.centos',
}
package { 'dhcp-libs':
  ensure => '4.2.5-47.el7.centos',
}
package { 'diffutils':
  ensure => '3.3-4.el7',
}
package { 'dmidecode':
  ensure => '3.0-2.1.el7_3',
}
package { 'dnsmasq':
  ensure => '2.66-21.el7',
}
package { 'docutils':
  ensure => '0.14',
}
package { 'dracut':
  ensure => '033-463.el7_3.2',
}
package { 'dracut-config-generic':
  ensure => '033-463.el7_3.2',
}
package { 'dracut-config-rescue':
  ensure => '033-463.el7_3.2',
}
package { 'dracut-network':
  ensure => '033-463.el7_3.2',
}
package { 'duo_unix':
  ensure => '1.9.21-0.el7',
}
package { 'e2fsprogs':
  ensure => '1.42.9-9.el7',
}
package { 'e2fsprogs-libs':
  ensure => '1.42.9-9.el7',
}
package { 'ebtables':
  ensure => '2.0.10-15.el7',
}
package { 'ed':
  ensure => '1.9-4.el7',
}
package { 'elfutils-libelf':
  ensure => '0.166-2.el7',
}
package { 'elfutils-libs':
  ensure => '0.166-2.el7',
}
package { 'epel-release':
  ensure => '7-10',
}
package { 'ethtool':
  ensure => '4.5-3.el7',
}
package { 'expat':
  ensure => '2.1.0-10.el7_3',
}
package { 'facter':
  ensure => '2.4.6-1.el7',
}
package { 'fail2ban':
  ensure => '0.9.6-3.el7',
}
package { 'fail2ban-firewalld':
  ensure => '0.9.6-3.el7',
}
package { 'fail2ban-sendmail':
  ensure => '0.9.6-3.el7',
}
package { 'fail2ban-server':
  ensure => '0.9.6-3.el7',
}
package { 'file':
  ensure => '5.11-33.el7',
}
package { 'file-libs':
  ensure => '5.11-33.el7',
}
package { 'filesystem':
  ensure => '3.2-21.el7',
}
package { 'findutils':
  ensure => '4.5.11-5.el7',
}
package { 'fipscheck':
  ensure => '1.4.1-5.el7',
}
package { 'fipscheck-lib':
  ensure => '1.4.1-5.el7',
}
package { 'firewalld':
  ensure => '0.4.3.2-8.1.el7_3.3',
}
package { 'firewalld-filesystem':
  ensure => '0.4.3.2-8.1.el7_3.3',
}
package { 'freetype':
  ensure => '2.4.11-12.el7',
}
package { 'futures':
  ensure => '3.1.1',
}
package { 'gawk':
  ensure => '4.0.2-4.el7_3.1',
}
package { 'gcc':
  ensure => '4.8.5-11.el7',
}
package { 'gdbm':
  ensure => '1.10-8.el7',
}
package { 'gettext':
  ensure => '0.18.2.1-4.el7',
}
package { 'gettext-libs':
  ensure => '0.18.2.1-4.el7',
}
package { 'glib-networking':
  ensure => '2.42.0-1.el7',
}
package { 'glib2':
  ensure => '2.46.2-4.el7',
}
package { 'glibc':
  ensure => '2.17-157.el7_3.5',
}
package { 'glibc-common':
  ensure => '2.17-157.el7_3.5',
}
package { 'glibc-devel':
  ensure => '2.17-157.el7_3.5',
}
package { 'glibc-headers':
  ensure => '2.17-157.el7_3.5',
}
package { 'gmp':
  ensure => '6.0.0-12.el7_1',
}
package { 'gnupg2':
  ensure => '2.0.22-4.el7',
}
package { 'gnutls':
  ensure => '3.3.24-1.el7',
}
package { 'gobject-introspection':
  ensure => '1.42.0-1.el7',
}
package { 'gpg-pubkey':
  ensure => 'f4a80eb5-53a7ff4b',
}
package { 'gpgme':
  ensure => '1.3.2-5.el7',
}
package { 'grep':
  ensure => '2.20-2.el7',
}
package { 'groff-base':
  ensure => '1.22.2-8.el7',
}
package { 'grub2':
  ensure => '2.02-0.44.el7.centos',
}
package { 'grub2-tools':
  ensure => '2.02-0.44.el7.centos',
}
package { 'grubby':
  ensure => '8.28-21.el7_3',
}
package { 'gsettings-desktop-schemas':
  ensure => '3.14.2-1.el7',
}
package { 'gssproxy':
  ensure => '0.4.1-13.el7',
}
package { 'gzip':
  ensure => '1.5-8.el7',
}
package { 'hardlink':
  ensure => '1.0-19.el7',
}
package { 'hiera':
  ensure => '1.3.4-5.el7',
}
package { 'hostname':
  ensure => '3.13-3.el7',
}
package { 'hwdata':
  ensure => '0.252-8.4.el7',
}
package { 'info':
  ensure => '5.1-4.el7',
}
package { 'iniparse':
  ensure => '0.4',
}
package { 'initscripts':
  ensure => '9.49.37-1.el7_3.1',
}
package { 'io-console':
  ensure => ['0.4.2'],
}
package { 'iproute':
  ensure => '3.10.0-74.el7',
}
package { 'ipset':
  ensure => '6.19-6.el7',
}
package { 'ipset-libs':
  ensure => '6.19-6.el7',
}
package { 'iptables':
  ensure => '1.4.21-17.el7',
}
package { 'iputils':
  ensure => '20160308-8.el7',
}
package { 'irqbalance':
  ensure => '1.0.7-6.el7_3.1',
}
package { 'iwl7265-firmware':
  ensure => '22.0.7.0-49.el7',
}
package { 'jansson':
  ensure => '2.4-6.el7',
}
package { 'jbigkit-libs':
  ensure => '2.0-11.el7',
}
package { 'jmespath':
  ensure => '0.9.3',
}
package { 'json':
  ensure => ['1.7.7'],
}
package { 'json-c':
  ensure => '0.11-4.el7_0',
}
package { 'jsonpatch':
  ensure => '1.2',
}
package { 'jsonpointer':
  ensure => '1.9',
}
package { 'kbd':
  ensure => '1.15.5-12.el7',
}
package { 'kbd-legacy':
  ensure => '1.15.5-12.el7',
}
package { 'kbd-misc':
  ensure => '1.15.5-12.el7',
}
package { 'kernel':
  ensure => '3.10.0-327.10.1.el7',
}
package { 'kernel-headers':
  ensure => '3.10.0-514.26.2.el7',
}
package { 'kernel-tools':
  ensure => '3.10.0-514.26.2.el7',
}
package { 'kernel-tools-libs':
  ensure => '3.10.0-514.26.2.el7',
}
package { 'kexec-tools':
  ensure => '2.0.7-50.el7',
}
package { 'keyutils':
  ensure => '1.5.8-3.el7',
}
package { 'keyutils-libs':
  ensure => '1.5.8-3.el7',
}
package { 'kitchen':
  ensure => '1.1.1',
}
package { 'kmod':
  ensure => '20-9.el7',
}
package { 'kmod-libs':
  ensure => '20-9.el7',
}
package { 'kpartx':
  ensure => '0.4.9-99.el7_3.3',
}
package { 'krb5-libs':
  ensure => '1.14.1-27.el7_3',
}
package { 'less':
  ensure => '458-9.el7',
}
package { 'libacl':
  ensure => '2.2.51-12.el7',
}
package { 'libassuan':
  ensure => '2.1.0-3.el7',
}
package { 'libattr':
  ensure => '2.4.46-12.el7',
}
package { 'libbasicobjects':
  ensure => '0.1.1-27.el7',
}
package { 'libblkid':
  ensure => '2.23.2-33.el7_3.2',
}
package { 'libcap':
  ensure => '2.22-8.el7',
}
package { 'libcap-ng':
  ensure => '0.7.5-4.el7',
}
package { 'libcgroup':
  ensure => '0.41-11.el7',
}
package { 'libcollection':
  ensure => '0.6.2-27.el7',
}
package { 'libcom_err':
  ensure => '1.42.9-9.el7',
}
package { 'libcroco':
  ensure => '0.6.8-5.el7',
}
package { 'libcurl':
  ensure => '7.29.0-35.el7.centos',
}
package { 'libdaemon':
  ensure => '0.14-7.el7',
}
package { 'libdb':
  ensure => '5.3.21-19.el7',
}
package { 'libdb-utils':
  ensure => '5.3.21-19.el7',
}
package { 'libedit':
  ensure => '3.0-12.20121213cvs.el7',
}
package { 'libestr':
  ensure => '0.1.9-2.el7',
}
package { 'libevent':
  ensure => '2.0.21-4.el7',
}
package { 'libffi':
  ensure => '3.0.13-18.el7',
}
package { 'libgcc':
  ensure => '4.8.5-11.el7',
}
package { 'libgcrypt':
  ensure => '1.5.3-13.el7_3.1',
}
package { 'libgomp':
  ensure => '4.8.5-11.el7',
}
package { 'libgpg-error':
  ensure => '1.12-3.el7',
}
package { 'libgudev1':
  ensure => '219-30.el7_3.9',
}
package { 'libidn':
  ensure => '1.28-4.el7',
}
package { 'libini_config':
  ensure => '1.3.0-27.el7',
}
package { 'libjpeg-turbo':
  ensure => '1.2.90-5.el7',
}
package { 'libmnl':
  ensure => '1.0.3-7.el7',
}
package { 'libmodman':
  ensure => '2.0.1-8.el7',
}
package { 'libmount':
  ensure => '2.23.2-33.el7_3.2',
}
package { 'libmpc':
  ensure => '1.0.1-3.el7',
}
package { 'libndp':
  ensure => '1.2-7.el7',
}
package { 'libnetfilter_conntrack':
  ensure => '1.0.6-1.el7_3',
}
package { 'libnfnetlink':
  ensure => '1.0.1-4.el7',
}
package { 'libnfsidmap':
  ensure => '0.25-15.el7',
}
package { 'libnl3':
  ensure => '3.2.28-3.el7_3',
}
package { 'libnl3-cli':
  ensure => '3.2.28-3.el7_3',
}
package { 'libpath_utils':
  ensure => '0.2.1-27.el7',
}
package { 'libpcap':
  ensure => '1.5.3-8.el7',
}
package { 'libpipeline':
  ensure => '1.2.3-3.el7',
}
package { 'libproxy':
  ensure => '0.4.11-10.el7',
}
package { 'libpwquality':
  ensure => '1.2.3-4.el7',
}
package { 'libref_array':
  ensure => '0.1.5-27.el7',
}
package { 'libselinux':
  ensure => '2.5-6.el7',
}
package { 'libselinux-python':
  ensure => '2.5-6.el7',
}
package { 'libselinux-ruby':
  ensure => '2.5-6.el7',
}
package { 'libselinux-utils':
  ensure => '2.5-6.el7',
}
package { 'libsemanage':
  ensure => '2.5-5.1.el7_3',
}
package { 'libsemanage-python':
  ensure => '2.5-5.1.el7_3',
}
package { 'libsepol':
  ensure => '2.5-6.el7',
}
package { 'libsoup':
  ensure => '2.48.1-6.el7',
}
package { 'libss':
  ensure => '1.42.9-9.el7',
}
package { 'libssh2':
  ensure => '1.4.3-10.el7_2.1',
}
package { 'libstdc++':
  ensure => '4.8.5-11.el7',
}
package { 'libsysfs':
  ensure => '2.1.0-16.el7',
}
package { 'libtalloc':
  ensure => '2.1.6-1.el7',
}
package { 'libtasn1':
  ensure => '3.8-3.el7',
}
package { 'libteam':
  ensure => '1.25-4.el7',
}
package { 'libtevent':
  ensure => '0.9.28-1.el7',
}
package { 'libtiff':
  ensure => '4.0.3-27.el7_3',
}
package { 'libtirpc':
  ensure => '0.2.4-0.8.el7_3',
}
package { 'libunistring':
  ensure => '0.9.3-9.el7',
}
package { 'libuser':
  ensure => '0.60-7.el7_1',
}
package { 'libutempter':
  ensure => '1.1.6-4.el7',
}
package { 'libuuid':
  ensure => '2.23.2-33.el7_3.2',
}
package { 'libverto':
  ensure => '0.2.5-4.el7',
}
package { 'libverto-tevent':
  ensure => '0.2.5-4.el7',
}
package { 'libwebp':
  ensure => '0.3.0-3.el7',
}
package { 'libxml2':
  ensure => '2.9.1-6.el7_2.3',
}
package { 'libxml2-python':
  ensure => '2.9.1-6.el7_2.3',
}
package { 'libyaml':
  ensure => '0.1.4-11.el7_0',
}
package { 'linux-firmware':
  ensure => '20160830-49.git7534e19.el7',
}
package { 'logrotate':
  ensure => '3.8.6-12.el7',
}
package { 'lua':
  ensure => '5.1.4-15.el7',
}
package { 'lzo':
  ensure => '2.06-8.el7',
}
package { 'm4':
  ensure => '1.4.16-10.el7',
}
package { 'mailx':
  ensure => '12.5-12.el7_0',
}
package { 'make':
  ensure => '3.82-23.el7',
}
package { 'man-db':
  ensure => '2.6.3-9.el7',
}
package { 'mariadb-libs':
  ensure => '5.5.52-1.el7',
}
package { 'microcode_ctl':
  ensure => '2.1-16.3.el7_3',
}
package { 'mig-agent':
  ensure => '20160715_0.a06734a.prod-1',
}
package { 'mozjs17':
  ensure => '17.0.0-19.el7',
}
package { 'mpfr':
  ensure => '3.1.1-4.el7',
}
package { 'ncurses':
  ensure => '5.9-13.20130511.el7',
}
package { 'ncurses-base':
  ensure => '5.9-13.20130511.el7',
}
package { 'ncurses-libs':
  ensure => '5.9-13.20130511.el7',
}
package { 'net-tools':
  ensure => '2.0-0.17.20131004git.el7',
}
package { 'nettle':
  ensure => '2.7.1-8.el7',
}
package { 'newt':
  ensure => '0.52.15-4.el7',
}
package { 'newt-python':
  ensure => '0.52.15-4.el7',
}
package { 'nfs-utils':
  ensure => '1.3.0-0.33.el7_3',
}
package { 'nspr':
  ensure => '4.13.1-1.0.el7_3',
}
package { 'nss':
  ensure => '3.28.4-1.2.el7_3',
}
package { 'nss-softokn':
  ensure => '3.16.2.3-14.4.el7',
}
package { 'nss-softokn-freebl':
  ensure => '3.16.2.3-14.4.el7',
}
package { 'nss-sysinit':
  ensure => '3.28.4-1.2.el7_3',
}
package { 'nss-tools':
  ensure => '3.28.4-1.2.el7_3',
}
package { 'nss-util':
  ensure => '3.28.4-1.0.el7_3',
}
package { 'ntp':
  ensure => '4.2.6p5-25.el7.centos.2',
}
package { 'ntpdate':
  ensure => '4.2.6p5-25.el7.centos.2',
}
package { 'numactl-libs':
  ensure => '2.0.9-6.el7_2',
}
package { 'openldap':
  ensure => '2.4.40-13.el7',
}
package { 'openssh':
  ensure => '6.6.1p1-35.el7_3',
}
package { 'openssh-clients':
  ensure => '6.6.1p1-35.el7_3',
}
package { 'openssh-server':
  ensure => '6.6.1p1-35.el7_3',
}
package { 'openssl':
  ensure => '1.0.1e-60.el7_3.1',
}
package { 'openssl-libs':
  ensure => '1.0.1e-60.el7_3.1',
}
package { 'os-prober':
  ensure => '1.58-9.el7',
}
package { 'p11-kit':
  ensure => '0.20.7-3.el7',
}
package { 'p11-kit-trust':
  ensure => '0.20.7-3.el7',
}
package { 'pam':
  ensure => '1.1.8-18.el7',
}
package { 'parted':
  ensure => '3.1-28.el7',
}
package { 'passwd':
  ensure => '0.79-4.el7',
}
package { 'patch':
  ensure => '2.7.1-8.el7',
}
package { 'pciutils':
  ensure => '3.5.1-1.el7',
}
package { 'pciutils-libs':
  ensure => '3.5.1-1.el7',
}
package { 'pcre':
  ensure => '8.32-15.el7_2.1',
}
package { 'perf':
  ensure => '0.1',
}
package { 'perl':
  ensure => '5.16.3-291.el7',
}
package { 'perl-Carp':
  ensure => '1.26-244.el7',
}
package { 'perl-Encode':
  ensure => '2.51-7.el7',
}
package { 'perl-Exporter':
  ensure => '5.68-3.el7',
}
package { 'perl-File-Path':
  ensure => '2.09-2.el7',
}
package { 'perl-File-Temp':
  ensure => '0.23.01-3.el7',
}
package { 'perl-Filter':
  ensure => '1.49-3.el7',
}
package { 'perl-Getopt-Long':
  ensure => '2.40-2.el7',
}
package { 'perl-HTTP-Tiny':
  ensure => '0.033-3.el7',
}
package { 'perl-NetAddr-IP':
  ensure => '4.069-3.el7',
}
package { 'perl-PathTools':
  ensure => '3.40-5.el7',
}
package { 'perl-Pod-Escapes':
  ensure => '1.04-291.el7',
}
package { 'perl-Pod-Perldoc':
  ensure => '3.20-4.el7',
}
package { 'perl-Pod-Simple':
  ensure => '3.28-4.el7',
}
package { 'perl-Pod-Usage':
  ensure => '1.63-3.el7',
}
package { 'perl-Scalar-List-Utils':
  ensure => '1.27-248.el7',
}
package { 'perl-Socket':
  ensure => '2.010-4.el7',
}
package { 'perl-Storable':
  ensure => '2.45-3.el7',
}
package { 'perl-Text-ParseWords':
  ensure => '3.29-4.el7',
}
package { 'perl-Time-HiRes':
  ensure => '1.9725-3.el7',
}
package { 'perl-Time-Local':
  ensure => '1.2300-2.el7',
}
package { 'perl-constant':
  ensure => '1.27-2.el7',
}
package { 'perl-libs':
  ensure => '5.16.3-291.el7',
}
package { 'perl-macros':
  ensure => '5.16.3-291.el7',
}
package { 'perl-parent':
  ensure => '0.225-244.el7',
}
package { 'perl-podlators':
  ensure => '2.5.1-3.el7',
}
package { 'perl-threads':
  ensure => '1.87-4.el7',
}
package { 'perl-threads-shared':
  ensure => '1.43-6.el7',
}
package { 'pinentry':
  ensure => '0.8.1-17.el7',
}
package { 'pkgconfig':
  ensure => '0.27.1-4.el7',
}
package { 'policycoreutils':
  ensure => '2.5-11.el7_3',
}
package { 'policycoreutils-default-encoding':
  ensure => '0.1',
}
package { 'policycoreutils-python':
  ensure => '2.5-11.el7_3',
}
package { 'polkit':
  ensure => '0.112-12.el7_3',
}
package { 'polkit-pkla-compat':
  ensure => '0.1-4.el7',
}
package { 'popt':
  ensure => '1.13-16.el7',
}
package { 'postfix':
  ensure => '2.10.1-6.el7',
}
package { 'ppp':
  ensure => '2.4.5-33.el7',
}
package { 'prettytable':
  ensure => '0.7.2',
}
package { 'procps-ng':
  ensure => '3.3.10-10.el7',
}
package { 'psmisc':
  ensure => '22.20-11.el7',
}
package { 'psych':
  ensure => ['2.0.0'],
}
package { 'pth':
  ensure => '2.0.7-23.el7',
}
package { 'puppet':
  ensure => '3.8.7-1.el7',
}
package { 'puppetlabs-release':
  ensure => '22.0-2',
}
package { 'pyasn1':
  ensure => '0.1.9',
}
package { 'pycrypto':
  ensure => '2.6.1',
}
package { 'pycurl':
  ensure => '7.19.0',
}
package { 'pygobject':
  ensure => '3.14.0',
}
package { 'pygobject3-base':
  ensure => '3.14.0-3.el7',
}
package { 'pygpgme':
  ensure => '0.3-9.el7',
}
package { 'pyliblzma':
  ensure => '0.5.3-11.el7',
}
package { 'python':
  ensure => '2.7.5-48.el7',
}
package { 'python-IPy':
  ensure => '0.75-6.el7',
}
package { 'python-backports':
  ensure => '1.0-8.el7',
}
package { 'python-backports-ssl_match_hostname':
  ensure => '3.4.0.2-4.el7',
}
package { 'python-chardet':
  ensure => '2.2.1-1.el7_1',
}
package { 'python-cheetah':
  ensure => '2.4.4-5.el7.centos',
}
package { 'python-configobj':
  ensure => '4.7.2-7.el7',
}
package { 'python-dateutil':
  ensure => '2.6.1',
}
package { 'python-decorator':
  ensure => '3.4.0-3.el7',
}
package { 'python-devel':
  ensure => '2.7.5-48.el7',
}
package { 'python-firewall':
  ensure => '0.4.3.2-8.1.el7_3.3',
}
package { 'python-iniparse':
  ensure => '0.4-9.el7',
}
package { 'python-jsonpatch':
  ensure => '1.2-3.el7.centos',
}
package { 'python-jsonpointer':
  ensure => '1.9-2.el7',
}
package { 'python-kitchen':
  ensure => '1.1.1-5.el7',
}
package { 'python-libs':
  ensure => '2.7.5-48.el7',
}
package { 'python-markdown':
  ensure => '2.4.1-1.el7.centos',
}
package { 'python-perf':
  ensure => '3.10.0-514.26.2.el7',
}
package { 'python-pillow':
  ensure => '2.0.0-19.gitd1c6db8.el7',
}
package { 'python-prettytable':
  ensure => '0.7.2-2.el7.centos',
}
package { 'python-pycurl':
  ensure => '7.19.0-19.el7',
}
package { 'python-pygments':
  ensure => '1.4-9.el7',
}
package { 'python-pyudev':
  ensure => '0.15-7.el7_2.1',
}
package { 'python-requests':
  ensure => '2.6.0-1.el7_1',
}
package { 'python-setuptools':
  ensure => '0.9.8-4.el7',
}
package { 'python-six':
  ensure => '1.9.0-2.el7',
}
package { 'python-slip':
  ensure => '0.4.0-2.el7',
}
package { 'python-slip-dbus':
  ensure => '0.4.0-2.el7',
}
package { 'python-urlgrabber':
  ensure => '3.10-8.el7',
}
package { 'python-urllib3':
  ensure => '1.10.2-2.el7_1',
}
package { 'python2-boto':
  ensure => '2.45.0-3.el7',
}
package { 'python2-pip':
  ensure => '8.1.2-5.el7',
}
package { 'python2-pyasn1':
  ensure => '0.1.9-7.el7',
}
package { 'python2-rsa':
  ensure => '3.4.1-1.el7',
}
package { 'pyudev':
  ensure => '0.15',
}
package { 'pyxattr':
  ensure => '0.5.1-5.el7',
}
package { 'qrencode-libs':
  ensure => '3.4.1-3.el7',
}
package { 'quota':
  ensure => '4.01-14.el7',
}
package { 'quota-nls':
  ensure => '4.01-14.el7',
}
package { 'rdma':
  ensure => '7.3_4.7_rc2-6.el7_3',
}
package { 'rdoc':
  ensure => ['4.0.0'],
}
package { 'readline':
  ensure => '6.2-9.el7',
}
package { 'redhat-lsb-core':
  ensure => '4.1-27.el7.centos.1',
}
package { 'redhat-lsb-submod-security':
  ensure => '4.1-27.el7.centos.1',
}
package { 'requests':
  ensure => '2.6.0',
}
package { 'rootfiles':
  ensure => '8.1-11.el7',
}
package { 'rpcbind':
  ensure => '0.2.0-38.el7_3.1',
}
package { 'rpm':
  ensure => '4.11.3-21.el7',
}
package { 'rpm-build-libs':
  ensure => '4.11.3-21.el7',
}
package { 'rpm-libs':
  ensure => '4.11.3-21.el7',
}
package { 'rpm-python':
  ensure => '4.11.3-21.el7',
}
package { 'rsa':
  ensure => '3.4.1',
}
package { 'rsync':
  ensure => '3.0.9-17.el7',
}
package { 'rsyslog':
  ensure => '7.4.7-16.el7',
}
package { 'ruby':
  ensure => '2.0.0.648-29.el7',
}
package { 'ruby-augeas':
  ensure => '0.5.0-1.el7',
}
package { 'ruby-devel':
  ensure => '2.0.0.648-29.el7',
}
package { 'ruby-irb':
  ensure => '2.0.0.648-29.el7',
}
package { 'ruby-libs':
  ensure => '2.0.0.648-29.el7',
}
package { 'ruby-shadow':
  ensure => '2.2.0-2.el7',
}
package { 'rubygem-bigdecimal':
  ensure => '1.2.0-29.el7',
}
package { 'rubygem-io-console':
  ensure => '0.4.2-29.el7',
}
package { 'rubygem-json':
  ensure => '1.7.7-29.el7',
}
package { 'rubygem-psych':
  ensure => '2.0.0-29.el7',
}
package { 'rubygem-rdoc':
  ensure => '4.0.0-29.el7',
}
package { 'rubygems':
  ensure => '2.0.14.1-29.el7',
}
package { 's3transfer':
  ensure => '0.1.10',
}
package { 'sed':
  ensure => '4.2.2-5.el7',
}
package { 'selinux-policy':
  ensure => '3.13.1-102.el7_3.16',
}
package { 'selinux-policy-targeted':
  ensure => '3.13.1-102.el7_3.16',
}
package { 'seobject':
  ensure => '0.1',
}
package { 'sepolicy':
  ensure => '1.1',
}
package { 'setools-libs':
  ensure => '3.3.8-1.1.el7',
}
package { 'setup':
  ensure => '2.8.71-7.el7',
}
package { 'shadow-utils':
  ensure => '4.1.5.1-24.el7',
}
package { 'shared-mime-info':
  ensure => '1.1-9.el7',
}
package { 'six':
  ensure => '1.9.0',
}
package { 'slang':
  ensure => '2.2.4-11.el7',
}
package { 'slip':
  ensure => '0.4.0',
}
package { 'slip.dbus':
  ensure => '0.4.0',
}
package { 'snappy':
  ensure => '1.1.0-3.el7',
}
package { 'spax':
  ensure => '1.5.2-13.el7',
}
package { 'sqlite':
  ensure => '3.7.17-8.el7',
}
package { 'sudo':
  ensure => '1.8.6p7-23.el7_3',
}
package { 'systemd':
  ensure => '219-30.el7_3.9',
}
package { 'systemd-libs':
  ensure => '219-30.el7_3.9',
}
package { 'systemd-python':
  ensure => '219-30.el7_3.9',
}
package { 'systemd-sysv':
  ensure => '219-30.el7_3.9',
}
package { 'sysvinit-tools':
  ensure => '2.88-14.dsf.el7',
}
package { 'tar':
  ensure => '1.26-31.el7',
}
package { 'tcp_wrappers':
  ensure => '7.6-77.el7',
}
package { 'tcp_wrappers-libs':
  ensure => '7.6-77.el7',
}
package { 'td-agent':
  ensure => '2.3.5-1.el2017',
}
package { 'teamd':
  ensure => '1.25-4.el7',
}
package { 'time':
  ensure => '1.7-45.el7',
}
package { 'trousers':
  ensure => '0.3.13-1.el7',
}
package { 'tuned':
  ensure => '2.7.1-3.el7_3.2',
}
package { 'tzdata':
  ensure => '2017b-1.el7',
}
package { 'unzip':
  ensure => '6.0-16.el7',
}
package { 'urlgrabber':
  ensure => '3.10',
}
package { 'urllib3':
  ensure => '1.10.2',
}
package { 'ustr':
  ensure => '1.0.4-16.el7',
}
package { 'util-linux':
  ensure => '2.23.2-33.el7_3.2',
}
package { 'vim-minimal':
  ensure => '7.4.160-1.el7_3.1',
}
package { 'virt-what':
  ensure => '1.13-8.el7',
}
package { 'wget':
  ensure => '1.14-13.el7',
}
package { 'which':
  ensure => '2.20-7.el7',
}
package { 'wpa_supplicant':
  ensure => '2.0-21.el7_3',
}
package { 'xfsprogs':
  ensure => '4.5.0-10.el7_3',
}
package { 'xz':
  ensure => '5.2.2-1.el7',
}
package { 'xz-libs':
  ensure => '5.2.2-1.el7',
}
package { 'yum':
  ensure => '3.4.3-150.el7.centos',
}
package { 'yum-metadata-parser':
  ensure => '1.1.4-10.el7',
}
package { 'yum-plugin-fastestmirror':
  ensure => '1.1.31-40.el7',
}
package { 'yum-utils':
  ensure => '1.1.31-40.el7',
}
package { 'zlib':
  ensure => '1.2.7-17.el7',
}
